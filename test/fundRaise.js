...
contract('FundRaise', function ([owner, donor]) {
  ...
  it('is able to accept funds', async function () {
    var donationAmount = 1e+18
    await fundRaise.sendTransaction({ value: donationAmount, from: donor })
    const fundRaiseAddress = await fundRaise.address
    assert.equal(web3.eth.getBalance(fundRaiseAddress).toNumber(), donationAmount)
  })
